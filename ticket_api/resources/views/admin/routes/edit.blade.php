...
<div class="bg-white shadow rounded-lg p-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('admin.routes.show', $route->id) }}" class="text-blue-500 hover:text-blue-700 flex items-center">
    ...
    <form action="{{ route('admin.routes.update', $route->id) }}" method="POST" novalidate>
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-gray-500">
                    <i class="fas fa-info-circle mr-1"></i> Perubahan status disini hanya mempengaruhi rute ini. Untuk memperbarui jadwal terkait, gunakan opsi di halaman detail.
                </p>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Confirm before status change if there are schedules
        const statusSelect = document.getElementById('status');
        const originalStatus = "{{ $route->status }}";

        statusSelect.addEventListener('change', function() {
            if (this.value !== originalStatus) {
                const hasSchedules = {{ Schedule::where('route_id', $route->id)->exists() ? 'true' : 'false' }};

                if (hasSchedules) {
                    const shouldChange = confirm('Perubahan status rute ini dapat mempengaruhi jadwal terkait. Apakah Anda ingin melanjutkan?');

...
@section('content')
    <div class="bg-white shadow rounded-lg overflow-hidden">
        ...
        <div class="p-6 bg-gradient-to-r from-indigo-50 to-purple-50 border-b">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <a href="{{ route('admin.schedules.create') }}"
                    class="bg-indigo-600 hover:bg-indigo-700 text-white py-2 px-4 rounded-lg transition shadow-sm hover:shadow flex items-center">
        ...
        <div class="px-6 pt-4">
            <div class="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 mb-4 rounded-r" role="alert">
                <div class="flex">
                    <div class="flex-shrink-0">
                        <i class="fas fa-cloud-showers-heavy text-yellow-500"></i>
                    </div>
                    <div class="ml-3">
                        ...
                        <p class="text-sm mt-1">
                            ...
                            @foreach($weatherIssueRoutes as $weatherRoute)
                                <a href="{{ route('admin.routes.show', $weatherRoute->id) }}" class="underline">
                                    {{ $weatherRoute->origin }} - {{ $weatherRoute->destination }}
                                </a>{{ !$loop->last ? ', ' : '' }}
                            @endforeach
                        </p>
                        <p class="text-sm mt-1">
                            Jadwal pada rute tersebut mungkin mengalami penundaan atau perubahan. Silakan cek detail rute untuk informasi lebih lanjut.
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="px-6 pb-4">
            <form action="{{ route('admin.schedules.index') }}" method="GET"
                ...
                <div class="flex items-center mt-4 justify-end gap-2">
                    <button type="submit"
                        class="bg-indigo-500 hover:bg-indigo-600 text-white py-2 px-4 rounded-lg transition flex items-center">
                        <i class="fas fa-filter mr-2"></i> Filter
                    </button>
                    @if ($search || $routeId || $ferryId || $status)
                        <a href="{{ route('admin.schedules.index') }}"
                            class="bg-gray-500 hover:bg-gray-600 text-white py-2 px-4 rounded-lg transition flex items-center">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col"
                                class="py-3 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Rute
                            </th>
                            <th scope="col"
                                class="py-3 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kapal
                            </th>
                            <th scope="col"
                                class="py-3 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Hari
                                Operasi</th>
                            <th scope="col"
                                class="py-3 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Waktu
                            </th>
                            <th scope="col"
                                class="py-3 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                Status</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($schedules as $schedule)
                            <tr class="hover:bg-gray-50 transition {{ $schedule->route->status != 'ACTIVE' ? 'bg-gray-50' : '' }}">
                                <td class="py-4 px-4 whitespace-nowrap text-sm text-gray-500">{{ $loop->iteration }}</td>
                                <td class="py-4 px-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div
                                            class="flex-shrink-0 h-8 w-8 {{ $schedule->route->status == 'WEATHER_ISSUE' ? 'bg-yellow-100' : ($schedule->route->status == 'INACTIVE' ? 'bg-red-100' : 'bg-indigo-100') }} rounded-full flex items-center justify-center mr-3">
                                            <i class="fas fa-route {{ $schedule->route->status == 'WEATHER_ISSUE' ? 'text-yellow-500' : ($schedule->route->status == 'INACTIVE' ? 'text-red-500' : 'text-indigo-500') }}"></i>
                                        </div>
                                        <div>
                                            <div class="text-sm font-medium text-gray-900">{{ $schedule->route->origin }} →
                                                {{ $schedule->route->destination }}</div>
                                            <div class="text-xs {{ $schedule->route->status != 'ACTIVE' ? 'text-red-500 font-semibold' : 'text-gray-500' }}">
                                ...
                                <td class="py-4 px-4 whitespace-nowrap">
                                    <div class="text-sm">
                                        <div class="flex items-center mb-1">
                                            <div class="relative w-5 h-5 text-green-500 mr-2 flex justify-center">
                                                <i class="fas fa-ship"></i>
                                                <i class="fas fa-arrow-right text-xs absolute bottom-0 right-0"></i>
                                            </div>
                                            <span>{{ $schedule->departure_time->format('H:i') }}</span>
                                        </div>
                                        <div class="flex items-center">
                                            <div class="relative w-5 h-5 text-red-500 mr-2 flex justify-center">
                                                <i class="fas fa-ship"></i>
                                                <i class="fas fa-anchor text-xs absolute bottom-0 right-0"></i>
                                            </div>
                            ...
                            <tr>
                                <td colspan="7" class="py-8 px-4 text-center text-gray-500">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-sm text-gray-500 mt-1">Silakan tambahkan jadwal baru dengan mengklik
                                            tombol "Tambah Jadwal Baru"</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse

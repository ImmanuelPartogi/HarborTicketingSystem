            ...
            <form action="{{ route('admin.bookings.index') }}" method="GET">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-5">
                    <div class="lg:col-span-1">
                        ...
                        <div class="relative">
                            <select id="status" name="status"
                                class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-3 pr-10 transition-all duration-200 hover:border-indigo-300 shadow-sm">
                    ...
                    <div class="lg:col-span-1">
                        ...
                        <div class="relative">
                            ...
                            <input type="date" id="date_from" name="date_from" value="{{ request('date_from') }}"
                                class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-10 p-3 transition-all duration-200 hover:border-indigo-300 shadow-sm">
                        </div>
                    </div>
                    <div class="lg:col-span-1">
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="search" name="search" value="{{ request('search') }}"
                                placeholder="No. Booking / Nama Penumpang"
                                class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-10 p-3 transition-all duration-200 hover:border-indigo-300 shadow-sm">
                            <div
                    ...
                    <div class="lg:col-span-1 flex items-end space-x-2">
                        <button type="submit"
                            class="text-white bg-indigo-600 hover:bg-indigo-700 focus:ring-4 focus:ring-indigo-300 font-medium rounded-lg text-sm px-5 py-3 flex items-center justify-center flex-grow transition-all duration-300 shadow-sm">
                            <i class="fas fa-filter mr-1.5"></i> Filter
                        </button>
                        <a href="{{ route('admin.bookings.index') }}"
                            class="text-gray-600 bg-gray-200 hover:bg-gray-300 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-4 py-3 flex items-center justify-center transition-all duration-300 shadow-sm"
                            title="Reset Filter">
        ...
        <div class="hidden md:block overflow-x-auto rounded-xl border border-gray-200 shadow-sm">
            <table class="min-w-full bg-white divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th
                            class="py-3.5 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-b border-gray-200">
                            No. Booking</th>
                        <th
                            class="py-3.5 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-b border-gray-200">
                            Pengguna</th>
                    ...
                    @forelse($bookings as $booking)
                        <tr class="hover:bg-gray-50 transition-colors duration-200">
                            <td class="py-3.5 px-4 text-sm">
                                <div class="flex items-center">
                                    <div
                                        class="w-8 h-8 rounded-lg bg-indigo-100 flex items-center justify-center mr-2 shadow-sm">
                            ...
                            <td class="py-3.5 px-4 text-sm">
                                <div class="flex items-center">
                                    <div
                                        class="w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center mr-2 text-gray-500">
                            ...
                            <td class="py-3.5 px-4 text-sm">
                                @if ($booking->status == 'pending')
                                    <span
                                        class="px-3 py-1.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 border border-yellow-200 flex items-center w-fit shadow-sm">
                                        <i class="fas fa-clock mr-1.5"></i> Menunggu
                                    </span>
                                @elseif($booking->status == 'confirmed')
                                    <span
                                        class="px-3 py-1.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 border border-blue-200 flex items-center w-fit shadow-sm">
                                        <i class="fas fa-check mr-1.5"></i> Terkonfirmasi
                                    </span>
                                @elseif($booking->status == 'completed')
                                    <span
                                        class="px-3 py-1.5 rounded-full text-xs font-medium bg-green-100 text-green-800 border border-green-200 flex items-center w-fit shadow-sm">
                                        <i class="fas fa-check-double mr-1.5"></i> Selesai
                                    </span>
                                @elseif($booking->status == 'cancelled')
                                    <span
                                        class="px-3 py-1.5 rounded-full text-xs font-medium bg-red-100 text-red-800 border border-red-200 flex items-center w-fit shadow-sm">
                            ...
                            <td class="py-3.5 px-4 text-sm text-gray-600">
                                <div class="flex flex-col">
                                    <span>{{ $booking->created_at ? $booking->created_at->format('d/m/Y') : 'N/A' }}</span>
                                    <span
                                        class="text-xs text-gray-500">{{ $booking->created_at ? $booking->created_at->format('H:i') : 'N/A' }}</span>
                                </div>
                            </td>
                            <td class="py-3.5 px-4 text-sm">
                                <div class="flex space-x-2">
                                    <a href="{{ route('admin.bookings.show', $booking->id) }}"
                                        class="text-blue-600 hover:text-blue-900 bg-blue-100 hover:bg-blue-200 p-2 rounded-lg transition-colors duration-200 shadow-sm"
                                        title="Lihat Detail">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ route('admin.bookings.tickets', $booking->id) }}"
                                        class="text-indigo-600 hover:text-indigo-900 bg-indigo-100 hover:bg-indigo-200 p-2 rounded-lg transition-colors duration-200 shadow-sm"
                                        title="Lihat Tiket">
                                    ...
                                    @if ($booking->status == 'pending')
                                        <form action="{{ route('admin.bookings.confirm', $booking->id) }}" method="POST"
                                            ...
                                            @csrf
                                            <button type="submit"
                                                class="text-green-600 hover:text-green-900 bg-green-100 hover:bg-green-200 p-2 rounded-lg transition-colors duration-200 shadow-sm"
                                        ...
                                        <form action="{{ route('admin.bookings.cancel', $booking->id) }}" method="POST"
                                            ...
                                            @csrf
                                            <button type="submit"
                                                class="text-red-600 hover:text-red-900 bg-red-100 hover:bg-red-200 p-2 rounded-lg transition-colors duration-200 shadow-sm"
            ...
            @forelse($bookings as $booking)
                <div class="bg-white border rounded-xl shadow-sm p-5 transition-all duration-300 hover:shadow-md">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        @if ($booking->status == 'pending')
                            <span
                                class="px-3 py-1.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 border border-yellow-200 flex items-center shadow-sm">
                                <i class="fas fa-clock mr-1.5"></i> Menunggu
                            </span>
                        @elseif($booking->status == 'confirmed')
                            <span
                                class="px-3 py-1.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 border border-blue-200 flex items-center shadow-sm">
                                <i class="fas fa-check mr-1.5"></i> Terkonfirmasi
                            </span>
                        @elseif($booking->status == 'completed')
                            <span
                                class="px-3 py-1.5 rounded-full text-xs font-medium bg-green-100 text-green-800 border border-green-200 flex items-center shadow-sm">
                                <i class="fas fa-check-double mr-1.5"></i> Selesai
                            </span>
                        @elseif($booking->status == 'cancelled')
                            <span
                                class="px-3 py-1.5 rounded-full text-xs font-medium bg-red-100 text-red-800 border border-red-200 flex items-center shadow-sm">
                                <i class="fas fa-times mr-1.5"></i> Dibatalkan
                            </span>
                        @endif
                    </div>
                    ...
                    <div class="flex flex-wrap justify-between items-center pt-4 border-t border-gray-200">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <a href="{{ route('admin.bookings.show', $booking->id) }}"
                                class="p-2 bg-blue-100 text-blue-600 hover:bg-blue-200 rounded-lg transition-colors duration-200 shadow-sm"
                                title="Lihat Detail">
                                <i class="fas fa-eye"></i>
                            </a>
                            <a href="{{ route('admin.bookings.tickets', $booking->id) }}"
                                class="p-2 bg-indigo-100 text-indigo-600 hover:bg-indigo-200 rounded-lg transition-colors duration-200 shadow-sm"
                                title="Lihat Tiket">
                                <i class="fas fa-ticket-alt"></i>
                            </a>
                            @if ($booking->status == 'pending')
                                <form action="{{ route('admin.bookings.confirm', $booking->id) }}" method="POST"
                                    ...
                                    @csrf
                                    <button type="submit"
                                        class="p-2 bg-green-100 text-green-600 hover:bg-green-200 rounded-lg transition-colors duration-200 shadow-sm"
                                        title="Konfirmasi Pembayaran">
                                        <i class="fas fa-check-circle"></i>
                                    </button>
                                </form>
                                <form action="{{ route('admin.bookings.cancel', $booking->id) }}" method="POST"
                                    ...
                                    @csrf
                                    <button type="submit"
                                        class="p-2 bg-red-100 text-red-600 hover:bg-red-200 rounded-lg transition-colors duration-200 shadow-sm"
                                        onclick="return confirm('Apakah Anda yakin ingin membatalkan pemesanan ini?')"
                                        title="Batalkan Pemesanan">
                                        <i class="fas fa-times-circle"></i>
                                    </button>
